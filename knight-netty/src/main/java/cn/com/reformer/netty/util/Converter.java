...
public class Converter {
    ...
    public static short bytes2UnsignedShort(byte[] b) {
        ...
        short targets = (short) ((b[0] & 0xff) | ((b[1] << 8) & 0xff00)); // | 表示安位或
        return targets;
    }
...
//	}
    ...
    public static String MD5UTF8(String src) {
        ...
        try {
            ...
            logger.error("MD5 utf-8 error", e);
            e.printStackTrace();
        }
        byte[] b = md.digest();
        des = bytes2Hexs(b);
    ...
    public static byte[] fillDataStrToByte(String source, int length) {
        byte[] ret = new byte[length];
        byte reulst;
        for (int i = 0; i < length; i++) {
            if (i < source.length()) {
                ret[i] = (byte) source.charAt(i);
            } else {
                ret[i] = 0x00;
            }
        }
